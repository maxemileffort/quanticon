...
cache_profile = 'fx' if IS_FOREX else 'cr'
cache_path = os.path.join(os.path.dirname(__file__), f"mtf_hmm_physics_{cache_profile}_cache.pkl")
cache_key = {
...
for s in [s for s in symbols[0]]:
    for n_r in n_regimes:
        ...
        # Filter for "HMM Bearish" + "Physics Short"
        ...
        run_label = f"{s} | {n_r}r" if len(n_regimes) > 1 else s
        ticker_run_data[run_label] = {
            'df': final_df.copy(),
...
pdf_output_path = os.path.join(os.path.dirname(__file__), "mtf_hmm_physics_eda_plots.pdf")

with PdfPages(pdf_output_path) as pdf:
    fig, ax = plt.subplots(figsize=(11, 8.5))
    sns.barplot(data=long_comparison, x='Ticker', y='Returns', hue='Dataset', ax=ax)
    ax.set_title("Consistency Check: Long Strategy Returns (Train vs Test)")
    ...
    fig, ax = plt.subplots(figsize=(11, 8.5))
    sns.barplot(data=full_results_df, x='Entry', y='Returns', hue='Dataset', ax=ax)
    ax.set_title("Consistency Check: Long Strategy Returns (Train vs Test)")
    ax.tick_params(axis='x', rotation=45)
    fig.tight_layout()
    pdf.savefig(fig, bbox_inches='tight')
    ...
    fig, ax = plt.subplots(figsize=(11, 8.5))
    sns.barplot(data=non_flats, x='Entry', y='Returns', hue='Dataset', ax=ax)
    ax.set_title("Consistency Check: Long Strategy Returns (Train vs Test)")
    ax.tick_params(axis='x', rotation=45)
    fig.tight_layout()
    pdf.savefig(fig, bbox_inches='tight')
    ...
    for df in proxy_dfs:
        ...
        return_metrics = ['Strategy_Ret', 'Bot_Net_Ret']

        # 1. Aggregate the returns (Sum) grouped by 'Dataset'
        # 2. Plot as a clustered bar chart
        fig, ax = plt.subplots(figsize=(11, 8.5))
        ret_df.groupby(['Entry', 'Dataset'])[return_metrics].sum().plot(
            kind='bar',
            ax=ax,
            width=0.8
        )
        ...
        # Formatting the plot
        ax.set_title(f'Aggregate Returns Comparison - {ticker}', fontsize=14)
    ...
    for run_label, run_data in ticker_run_data.items():
        tdf = run_data['df'].sort_index().copy()
        # tdf = tdf.tail(50)
        split_dt = run_data['split_date']

        net_equity = (1 + tdf['Net_Ret'].fillna(0.0)).cumprod()
        gross_equity = (1 + tdf['Strategy_Ret'].fillna(0.0)).cumprod()
        buy_hold_equity = (1 + tdf['Returns'].fillna(0.0)).cumprod()

        fig, ax = plt.subplots(figsize=(11, 8.5))
        ax.plot(net_equity.index, net_equity.values, label='Net Strategy Equity', linewidth=1.5)
        ax.plot(gross_equity.index, gross_equity.values, label='Gross Strategy Equity', linewidth=1.2, alpha=0.8)
        ax.plot(buy_hold_equity.index, buy_hold_equity.values, label='Buy & Hold Equity', linewidth=1.2, alpha=0.8)
        # ax.axvline(split_dt, color='black', linestyle='--', linewidth=1.0, alpha=0.7, label='Train/Test Split')
        ax.set_title(f'Equity Curves - {run_label}')
        ax.set_xlabel('Time')
    ...
    # Combined equal-weight equity curve across all tickers
    if len(ticker_run_data) > 0:
        net_rets = []
        bh_rets = []
        for run_label, run_data in ticker_run_data.items():
            tdf = run_data['df'].sort_index()
            # tdf = tdf.tail(50)
            net_rets.append(tdf['Net_Ret'].rename(run_label))
            bh_rets.append(tdf['Returns'].rename(run_label))

        net_rets_df = pd.concat(net_rets, axis=1).sort_index()
        bh_rets_df = pd.concat(bh_rets, axis=1).sort_index()

        combined_net_ret = net_rets_df.mean(axis=1, skipna=True).fillna(0.0)
        combined_bh_ret = bh_rets_df.mean(axis=1, skipna=True).fillna(0.0)

        combined_net_equity = (1 + combined_net_ret).cumprod()
        combined_bh_equity = (1 + combined_bh_ret).cumprod()

        fig, ax = plt.subplots(figsize=(11, 8.5))
        ax.plot(combined_net_equity.index, combined_net_equity.values, label='Combined Net Strategy', linewidth=1.7)
        ax.plot(combined_bh_equity.index, combined_bh_equity.values, label='Combined Buy & Hold', linewidth=1.3, alpha=0.85)
        ax.set_title('Combined Equity Curve (Equal-Weight Across Tickers)')
        ax.set_xlabel('Time')
        ax.set_ylabel('Equity (Start = 1.0)')
        ax.grid(axis='y', linestyle='--', alpha=0.5)
        ax.legend(loc='best')
    ...
    # Price chart with trade markers per ticker
    for run_label, run_data in ticker_run_data.items():
        tdf = run_data['df'].sort_index().copy()
        # tdf = tdf.tail(50)
        ...
        price_series = tdf[('Open', ticker)].copy()
        entry_change = tdf['Entry'] != tdf['Entry'].shift(1)
        long_entries = entry_change & (tdf['Entry'] == 'Long')
        short_entries = entry_change & (tdf['Entry'] == 'Short')
        exits = entry_change & (tdf['Entry'].shift(1).isin(['Long', 'Short']))

        fig, ax = plt.subplots(figsize=(11, 8.5))
        ax.plot(price_series.index, price_series.values, color='steelblue', linewidth=1.1, label='Open Price')

        ax.scatter(
            price_series.index[long_entries],
            price_series[long_entries],
            marker='^',
            color='green',
            s=40,
            label='Long Entry'
        ...
        ax.scatter(
            price_series.index[short_entries],
            price_series[short_entries],
            marker='v',
            color='red',
            s=40,
            label='Short Entry'
        ...
        # ax.axvline(split_dt, color='black', linestyle='--', linewidth=1.0, alpha=0.7, label='Train/Test Split')
        ax.set_title(f'Price + Trade Markers - {run_label}')
        ax.set_xlabel('Time')
        ax.set_ylabel('Price')
...
print(f"Saved plots PDF: {pdf_output_path}")
