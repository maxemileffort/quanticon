...
def is_page_too_old(html_content: str) -> bool:
    """
    Assesses if the page content is older than 6 months by looking for dates.
    Returns True if the most recent date found is older than 6 months, False otherwise.
    """
    soup = BeautifulSoup(html_content, 'html.parser')
    text = soup.get_text()

    # Regex to find common date patterns (e.g., YYYY-MM-DD, MM/DD/YYYY, Month DD, YYYY)
    # This is a basic attempt and might need refinement based on target websites
    date_patterns = [
        r'\b\d{4}-\d{2}-\d{2}\b',  # YYYY-MM-DD
        r'\b\d{1,2}/\d{1,2}/\d{4}\b',  # MM/DD/YYYY or M/D/YYYY
        r'\b(?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{1,2}, \d{4}\b', # Month DD, YYYY
        r'\b\d{1,2} (?:Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[a-z]* \d{4}\b', # DD Month YYYY
        r'\b\d{4}\b' # Just a year, as a last resort
    ]
    ...
    for pattern in date_patterns:
        for match in re.finditer(pattern, text, re.IGNORECASE):
            try:
                ...
                dt = date_parse(match.group(0), fuzzy=True)
                # Ignore dates that are clearly not publication dates (e.g., future dates, very old dates)
                if datetime.now() - timedelta(days=365*5) < dt < datetime.now() + timedelta(days=1):
                    found_dates.append(dt)
            except ParserError:
                continue
            except OverflowError: # Handle dates too far in the past/future for datetime
                continue

    if not found_dates:
        # If no dates are found, we can't determine age, so don't skip
        return False

    most_recent_date = max(found_dates)
    six_months_ago = datetime.now() - timedelta(days=6 * 30) # Approximately 6 months

    return most_recent_date < six_months_ago
...
class PlaywrightCrawler:
    ...
    async def _visit_page(self, page, url: str) -> tuple[bool, str | None]:
        ...
        try:
            ...
            if is_page_too_old(html_content):
                print(f"[SKIP] Page content is older than 6 months: {url}")
                return False, None
    ...
    async def crawl(self):
        ...
        async with async_playwright() as p:
            ...
            try:
                ...
                while queue:
                    print(f"[PROGRESS] Links visited: {len(self.visited)}")
                    print(f"[PROGRESS] Links remaining in queue: {len(queue)}")
